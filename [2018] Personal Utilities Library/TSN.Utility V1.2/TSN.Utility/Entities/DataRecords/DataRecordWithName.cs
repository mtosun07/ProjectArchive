...
namespace TSN.Utility.Entities.DataRecords
{
    ...
    public abstract class DataRecordWithName<TDerived> : DataRecord<TDerived>, IName
        where TDerived : DataRecordWithName<TDerived>
    {
        public DataRecordWithName(Guid ID, string name)
            : base(ID)
        {
            _name = name?.Clone<string>() ?? throw new ArgumentNullException(nameof(name));
        }
        protected DataRecordWithName(SerializationInfo info, StreamingContext context)
            : base(info, context)
        {
