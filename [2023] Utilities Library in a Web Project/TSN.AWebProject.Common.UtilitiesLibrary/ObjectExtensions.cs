...
namespace TSN.AWebProject.Common.UtilitiesLibrary
{
    public static class ObjectExtensions
    {
        ...
        public static DisplayAttribute GetDisplayAttribute(this Enum e) => e.GetType().GetMember(e.ToString()).FirstOrDefault()?.GetCustomAttributes(false)?.Select(x => x as DisplayAttribute).Where(x => x != null).FirstOrDefault();
        ...
        public static bool TryParseToCultureInfo(string s, out CultureInfo ci)
        {
            try
            {
                return (ci = CultureInfo.GetCultureInfo(s)) != null;
            }
